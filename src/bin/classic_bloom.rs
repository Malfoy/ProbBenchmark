...
type ClassicFilter = ClassicBloomFilter<DefaultBuildHashKernels<DefaultBuildHasher>>;
...
#[command(
    author,
    version,
    about = "Indexes k-mers from one FASTA into a Bloom filter and queries k-mers from another FASTA with bloom-filters classic Bloom."
)]
struct Cli {
    #[command(flatten)]
    common: CommonArgs,
...
fn run_benchmark(args: &CommonArgs) -> Result<BenchmarkReport> {
    ...
    let requested_bits = resolve_bloom_bits(args, indexed_kmers)?;
    let bloom_hashes = match args.hashes {
        ...
        None => {
            ...
            optimal_num_hashes(requested_bits, expected_items)
        }
    };

    let mut bloom = build_filter(requested_bits, bloom_hashes);
    let bloom_bits = bloom.buckets().len();
...
fn build_filter(bloom_bits: usize, bloom_hashes: u32) -> ClassicFilter {
    let raw_data = vec![0_u8; bloom_bits.div_ceil(8)];
    ClassicBloomFilter::with_raw_data(
        &raw_data,
        bloom_hashes as usize,
...
fn index_fasta(
    ...
    k: usize,
    batch_bases: usize,
    global_filter: &mut ClassicFilter,
) -> Result<u64> {
    let mut total = 0_u64;
    ...
    scan_fasta_batches(path, batch_bases, |batch| {
        let batch_total: u64 = batch
            ...
            .map(|seq| {
                ...
                let mut filter = shared_filter.lock().expect("classic bloom mutex poisoned");
    ...
    *global_filter = Arc::into_inner(shared_filter)
        .expect("classic bloom filter still has multiple owners")
        .into_inner()
        .expect("classic bloom mutex poisoned");
...
#[cfg(test)]
mod tests {
    use super::{ClassicFilter, build_filter};
    use bloom_filters::BloomFilter;
    use probbenchmark::visit_sequence_kmers;

    #[test]
    fn classic_filter_reports_inserted_hits() {
        let mut filter: ClassicFilter = build_filter(1024, 3);
